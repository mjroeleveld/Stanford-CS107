...
static void ReadStopWords(hashset *words);
static void Welcome(const char *welcomeTextFileName);
static void BuildIndices(const char *feedsFileName, Index *index, ArticleSet *articles, StringSet *stopWords);
static void ProcessFeed(const char *remoteDocumentName, Index *index, ArticleSet *articles, StringSet *stopWords);
static void PullAllNewsItems(urlconnection *urlconn, Index *index, ArticleSet *articles, StringSet *stopWords);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, Index *index, ArticleSet *articles, StringSet *stopWords);
static void ExtractElement(streamtokenizer *st, const char *htmlTag, char dataBuffer[], int bufferLength);
static void ParseArticle(Article *article, Index *index, StringSet *stopWords);
static void ScanArticle(streamtokenizer *st, Article *article, Index *index, StringSet *stopWords);
static void QueryIndices(Index *index);
static void ProcessResponse(const char *word, Index *index);
...
int main(int argc, char **argv)
{
  ...
  HashSetNew(&articles, sizeof(Article), 1009,
             ArticleSetHashFunction, ArticleSetCompareFunction, ArticleSetFreeFunction);
  HashSetNew(&stopWords, sizeof(char*), 1009,
             StringSetHashFunction, StringSetCompareFunction, StringSetFreeFunction);
  IndexNew(&index, 10007);

  ReadStopWords(&stopWords);
  
  const char* feedsFileName = (argc == 1) ? kDefaultFeedsFile : argv[1];
  BuildIndices(feedsFileName, &index, &articles, &stopWords);
  QueryIndices(&index);
...
static void BuildIndices(const char *feedsFileName, Index *index, ArticleSet *articles, StringSet *stopWords)
{
  ...
  while (STSkipUntil(&st, ":") != EOF) { // ignore everything up to the first selicolon of the line
    ...
    ProcessFeed(remoteFileName, index, articles, stopWords);
  }
...
static void ProcessFeed(const char *remoteDocumentName, Index *index, ArticleSet *articles, StringSet *stopWords)
{
  ...
  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
              break;
      case 200: PullAllNewsItems(&urlconn, index, articles, stopWords);
                break;
      case 301:
      case 302: ProcessFeed(urlconn.newUrl, index, articles, stopWords);
                break;
...
static void PullAllNewsItems(urlconnection *urlconn, Index *index, ArticleSet *articles, StringSet *stopWords)
{
  ...
  while (GetNextItemTag(&st)) { // if true is returned, then assume that <item ...> has just been read and pulled from the data stream
    ProcessSingleNewsItem(&st, index, articles, stopWords);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, Index *index, ArticleSet *articles, StringSet *stopWords)
{
  ...
  strcpy(article->title, articleTitle);
  strcpy(article->url, articleURL);
  strcpy(article->description, articleDescription);
  HashSetEnter(articles, &article);

  ParseArticle(article, index, stopWords);
}
...
static void ParseArticle(Article *article, Index *index, StringSet *stopWords)
{
  ...
  URLNewAbsolute(&u, article->url);
  URLConnectionNew(&urlconn, &u);

  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Domain name or IP address is nonexistent.\n", article->url);
	      break;
      case 200: printf("Scanning \"%s\"", article->url, u.serverName);
        STNew(&st, urlconn.dataStream, kTextDelimiters, false);
        ScanArticle(&st, article, index, stopWords);
        STDispose(&st);
        break;
      case 301:
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
        ParseArticle(article, index, stopWords);
        break;
      default: printf("Unable to pull \"%s\" from \"%s\". [Response code: %d] Punting...\n", article->title, u.serverName, urlconn.responseCode);
        break;
  }
...
static void ScanArticle(streamtokenizer *st, Article *article, Index *index, StringSet *stopWords)
{
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      char* wordStr = strcat(word, "\0");
      if (WordIsWellFormed(word) && HashSetLookup(stopWords, &wordStr) == NULL) {
        IndexEnter(index, word, article);
        numWords++;
...
static void ProcessResponse(const char *word, Index *index)
{
  if (WordIsWellFormed(word)) {
    IndexItem *indexed = IndexLookup(index, word);
    if (indexed == NULL) {
      printf("\tNOT FOUND\n");
      return;
    }
    printf("\tResult articles:");
    VectorMap(indexed->articles, PrintArticle, NULL);
    printf("\n");
  } else {
...
static void PrintArticle(void *elemAddr, void *auxData)
{
  Article *article = *((Article**) elemAddr);
  printf("\n\t%s", article->url);
}
