...
struct film imdb::getFilm(const void *movieFile, int offset)
{
  ...
  string title(movie);
  int year = *(movie + title.length() + 2);
  const struct film f = { title, year + 1900 };
  return f;
}
...
bool imdb::getCredits(const string& player, vector<film>& films) const
{
  ...
  if (actor == nullptr)
    return false;
  short* starredNo = (short*)(actor + player.length() + 1);
  for (short i = 0; i < *starredNo; i++) {
    int* offset = (int*)(starredNo + 1) + i;
...
bool imdb::getCast(const film& movie, vector<string>& players) const
{
  ...
  if (found == nullptr)
    return false;
  char* year = found + movie.title.length() + 1;
  short* noPlayers = (short*)(year + 1);
  for (int i = 0; i < *noPlayers; i++) {
    int* offset = (int*)(year + 3) + i;
